...
if TYPE_CHECKING:
    ...
    class CustomizedPage:
        def __class_getitem__(cls, item: Any) -> Any:
            ...
            assert issubclass(
                page_cls, AbstractPage
            ), f"Expected subclass of AbstractPage, got {page_cls!r}"
...
@no_type_check
def _update_params_fields(
    cls: Type[AbstractParams], fields: ClsNamespace
) -> ClsNamespace:
    ...
    return {
        name: (_get_ann(name, val), _wrap_val(name, val))
...
class UseOptionalParams(PageCustomizer):
    def customize_page_ns(self, page_cls: PageCls, ns: ClsNamespace) -> None:
        ...
        new_fields = {
            name: _make_field_optional(field) for name, field in fields.items()
        }
