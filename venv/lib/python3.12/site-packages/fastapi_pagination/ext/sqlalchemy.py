...
def _should_unwrap_scalars(query: Selectable) -> bool:
    try:
        return (
            len(query.column_descriptions) == 1 and len(query._all_selected_columns) > 1
        )
...
def _paginate_from_statement(
    query: FromStatement, params: AbstractParams
) -> FromStatement:
...
def _maybe_unique(result: Any, unique: bool) -> Any:
    try:
        ...
        if "non-hashable" in str(e):
            raise NonHashableRowsException(
                "The rows are not hashable, please use `unique=False`"
            ) from e
