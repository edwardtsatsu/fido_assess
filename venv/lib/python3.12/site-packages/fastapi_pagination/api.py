...
_items_transformer_val: ContextVar[Optional[ItemsTransformer]] = ContextVar(
    "_items_transformer_val", default=None
)
...
def pagination_items() -> Sequence[Any]:
    try:
        ...
        raise RuntimeError(
            "pagination_items must be called inside create_page"
        ) from None
...
def apply_items_transformer(
    ...
    *,
    ...
    if is_coro and not async_:
        raise ValueError(
            "apply_items_transformer called with async_=False but transformer is async"
        )
...
def pagination_ctx(
    ...
    params_dep: Any = (
        _create_params_dependency(params) if params is not None else _noop_dep
    )
